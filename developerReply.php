...
$developers = $_SESSION['loginsession'];
...
<div class="d-flex" id="wrapper">
	<div class="bg-light" id="sidebar-wrapper" style="overflow-y: scroll;width: 23.3%; height: 1px;">
		...
		<div class="list-group list-group-flush">
			<?php
					$info = "SELECT DISTINCT m_from FROM managermessage WHERE m_to = '$developers' ORDER BY status ASC";
					$squery = $conn->query($info);
					while ($row = mysqli_fetch_array($squery)) {
						$adminName = "SELECT * FROM admin WHERE email = '$row[m_from]'";
						$adminNameQuery = $conn->query($adminName);
						$adminNameQueryCount = mysqli_num_rows($adminNameQuery);
						$ManagerName = "SELECT * FROM manager WHERE email = '$row[m_from]'";
						$ManagerQuery = $conn->query($ManagerName);
						$ManagerQueryCount = mysqli_num_rows($ManagerQuery);
			?>
			<a class="list-group-item list-group-item-action bg-light" href="developerReply.php?m_from=<?php echo $row["m_from"]; ?>">
				<?php
					
							if($adminNameQueryCount > 0){
							while ($adminNameQueryRow = mysqli_fetch_array($adminNameQuery)) {
					echo $adminNameQueryRow['fullName'];
					$m_from = $adminNameQueryRow['email'];
				}
				}
				elseif($ManagerQueryCount > 0){
				while ($ManagerNameQueryRow = mysqli_fetch_array($ManagerQuery)) {
					echo $ManagerNameQueryRow['fullName'];
				}
				}
				else{
				echo $row['m_from'];
					...
					<div class="card bg-light cardAdminReply" style=" border: none; display: none;">
						...
						<div class="card bg-light cardAdminReply" style=" border: none;">
							<div class="card-header header-heading">
								<?php
								$headName = "SELECT * FROM manager WHERE email = '$_GET[m_from]'";
								$headNameQuery = $conn->query($headName);
								$headNameQueryCount = mysqli_num_rows($headNameQuery);
								$headEmployeeName = "SELECT * FROM admin WHERE email = '$_GET[m_from]'";
							...
							<div class="card-body cardBody">
								<?php
								$updateMessage = "UPDATE managermessage SET status = 'Yes' WHERE m_from = '$_GET[m_from]' and m_to = '$developers'";
								...
								if ($messageRow['m_from'] == $_GET['m_from'] && $messageRow['m_to'] == $developers) {
								?>
								<div class="row">
								...
								elseif ($messageRow['m_from'] == $developers && $messageRow['m_to'] == $_GET['m_from'])
								{
								?>
								<div class="row">
									<div class="col-md-6"></div>
...
<?php
	...
	if (isset($_POST['submit'])) {
		if ($_POST['reply']!=null) {
		$quInsert = "INSERT INTO managermessage(message_id, m_to, m_from, message, sentDate, status) VALUES ('','$_GET[m_from]','$developers','$_POST[reply]',CURDATE(),'No')";
		$runQuery = $conn->query($quInsert);
		if ($runQuery) {
?>
<script type="text/javascript">
var val = "<?php echo $_GET['m_from'] ?>";
window.location="developerReply.php?m_from="+val;
</script>
<?php
	}
	else
...
$quInsert = "INSERT INTO managermessage(message_id, m_to, m_from, message, sentDate, status) VALUES ('','$_POST[replyEmail]','$developers','$_POST[replyMessage]',CURDATE(),'No')";
$runQuery = $conn->query($quInsert);
if ($runQuery) {
?>
<script type="text/javascript">
