...
<div class="d-flex" id="wrapper">
	<div class="bg-light" id="sidebar-wrapper" style="overflow-y: scroll;width: 23.3%; height: 1px;">
		...
		<div class="list-group list-group-flush">
			<?php
					$info = "SELECT DISTINCT m_from FROM managermessage WHERE m_to = '$admin' ORDER BY status ASC";
			...
			?>
			<a class="list-group-item list-group-item-action bg-light" href="adminReply.php?m_from=<?php echo $row["m_from"]; ?>">
				<?php
					...
					<div class="card bg-light cardAdminReply" style=" border: none; display: none;">
						...
						<div class="card bg-light cardAdminReply" style=" border: none;">
							...
							<div class="card-body cardBody">
								<?php
								$updateMessage = "UPDATE managermessage SET status = 'Yes' WHERE m_from = '$_GET[m_from]' and m_to = '$admin'";
								...
								if ($messageRow['m_from'] == $_GET['m_from'] && $messageRow['m_to'] == $admin) {
								?>
								<div class="row">
								...
								elseif ($messageRow['m_from'] == $admin && $messageRow['m_to'] == $_GET['m_from'])
								{
								?>
								<div class="row">
									<div class="col-md-6"></div>
...
if (isset($_POST['submit'])) {
	if ($_POST['reply']!=null) {
	$quInsert = "INSERT INTO managermessage(message_id, m_to, m_from, message, sentDate, status) VALUES ('','$_GET[m_from]','$admin','$_POST[reply]',CURDATE(),'No')";
	$runQuery = $conn->query($quInsert);
	if ($runQuery) {
?>
<script type="text/javascript">
var val = "<?php echo $_GET['m_from'] ?>";
window.location="adminReply.php?m_from="+val;
</script>
<?php
}
else
...
$quInsert = "INSERT INTO managermessage(message_id, m_to, m_from, message, sentDate, status) VALUES ('','$_POST[replyEmail]','$admin','$_POST[replyMessage]',CURDATE(),'No')";
$runQuery = $conn->query($quInsert);
if ($runQuery) {
?>
<script type="text/javascript">
	alert("Sent");
	window.location="adminReply.php";
</script>
<?php
}
else
